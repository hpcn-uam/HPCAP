 ...
 **/
#ifdef DEV_HPCAP
/*static*/ bool ixgbe_clean_tx_irq_hpcap(struct ixgbe_q_vector *q_vector, struct ixgbe_ring *tx_ring )
{
...
/*static*/ bool ixgbe_clean_tx_irq_standard(struct ixgbe_q_vector *q_vector, struct ixgbe_ring *tx_ring)
#else /* DEV_HPCAP*/
...
/*static*/ bool ixgbe_clean_tx_irq(struct ixgbe_q_vector *q_vector, struct ixgbe_ring *tx_ring )
{
	...
	if( adapter->work_mode == 2 )
		ret = ixgbe_clean_tx_irq_hpcap(q_vector, tx_ring);
	else
		ret = ixgbe_clean_tx_irq_standard(q_vector, tx_ring);
	return ret;
}
#endif /*DEV_HPCAP */
				    ...
				    struct ixgbe_rx_buffer *bi)
{
	...
	if (!page) {
		...
		#else
			page = alloc_pages(GFP_ATOMIC | __GFP_COLD | __GFP_COMP, ixgbe_rx_pg_order(rx_ring));
		#endif
		bi->page = page;
		if (unlikely(!page)) {
...
#ifdef DEV_HPCAP
void ixgbe_alloc_rx_buffers_standard(struct ixgbe_ring *rx_ring, u16 cleaned_count)
#else /* DEV_HPCAP */
 ...
 **/
/*static */void ixgbe_alloc_rx_buffers_hpcap(struct ixgbe_ring *rx_ring, u16 cleaned_count) //different from version 2.0.38
{
	union ixgbe_adv_rx_desc *rx_desc;
...
void ixgbe_alloc_rx_buffers(struct ixgbe_ring *rx_ring, u16 cleaned_count)
{
	...
	if( adapter->work_mode == 2 )
		ixgbe_alloc_rx_buffers_hpcap(rx_ring, cleaned_count);
	else
		ixgbe_alloc_rx_buffers_standard(rx_ring, cleaned_count);
}
#endif /* DEV_HPCAP */
...
#endif
	#if ( LINUX_VERSION_CODE < KERNEL_VERSION(3,5,0) )
		va = kmap_atomic(skb_frag_page(frag), KM_SKB_DATA_SOFTIRQ) + frag->page_offset;
	#else
		va = kmap_atomic(skb_frag_page(frag)) + frag->page_offset;
	#endif
				  ...
				  int budget)
{
	...
	while (ixgbe_test_staterr(rx_desc, IXGBE_RXD_STAT_DD)) {
		...
		#else
				le16_to_cpu(rx_desc->wb.upper.length), ALIGN(le16_to_cpu(rx_desc->wb.upper.length), L1_CACHE_BYTES) );
		#endif
...
#else
/*static*/ int ixgbe_vlan_rx_add_vid(struct net_device *netdev/*, __always_unused __be16 proto*/, u16 vid)
#endif
{
	struct ixgbe_adapter *adapter = netdev_priv(netdev);
	struct ixgbe_hw *hw = &adapter->hw;
...
#else
/*static*/ int ixgbe_vlan_rx_kill_vid(struct net_device *netdev/*, __always_unused __be16 proto,*/, u16 vid)
#endif
{
	struct ixgbe_adapter *adapter = netdev_priv(netdev);
	struct ixgbe_hw *hw = &adapter->hw;
...
int ixgbe_setup_rx_resources(struct ixgbe_ring *rx_ring)
{
	...
	#ifdef DEV_HPCAP
		rx_ring->queued = 0;
		size = ALIGN(IXGBE_SUBWINDOW_SIZE * MAX_DESCR_SIZE, 4096);
