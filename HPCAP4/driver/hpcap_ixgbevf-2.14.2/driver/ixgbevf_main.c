 ...
 **/
#ifdef DEV_HPCAP
/*static*/ bool ixgbevf_clean_tx_irq_hpcap(struct ixgbevf_q_vector *q_vector, struct ixgbevf_ring *tx_ring )
{
...
#define TX_WAKE_THRESHOLD (DESC_NEEDED * 2)
	if( unlikely( count && netif_carrier_ok(netdev) && ( ixgbevf_desc_unused(tx_ring) >= TX_WAKE_THRESHOLD ) ) ) // different from version 2.0.38
	{
		...
		smp_mb();
#ifdef HAVE_TX_MQ
		if( __netif_subqueue_stopped(netdev, tx_ring->queue_index) && !test_bit(__IXGBEVF_DOWN, &adapter->state))
		{
			netif_wake_subqueue(netdev, tx_ring->queue_index);
			++adapter->restart_queue;
		}
#else
		if( netif_queue_stopped(netdev) && !test_bit(__IXGBEVF_DOWN, &adapter->state))
		{
...
static bool ixgbevf_clean_tx_irq_standard(struct ixgbevf_q_vector *q_vector, struct ixgbevf_ring *tx_ring)
#else /* DEV_HPCAP */
...
/*static*/ bool ixgbevf_clean_tx_irq(struct ixgbevf_q_vector *q_vector, struct ixgbevf_ring *tx_ring )
{
	struct ixgbevf_adapter *adapter = tx_ring->adapter;
	bool ret;
	if( adapter->work_mode == 2 )
		ret = ixgbevf_clean_tx_irq_hpcap(q_vector, tx_ring);
	else
		ret = ixgbevf_clean_tx_irq_standard(q_vector, tx_ring);
	return ret;
}
#endif /*DEV_HPCAP */
...
/*static*/
#ifdef DEV_HPCAP
void ixgbevf_alloc_rx_buffers_standard(struct ixgbevf_ring *rx_ring, u16 cleaned_count)
#else /* DEV_HPCAP */
void ixgbevf_alloc_rx_buffers(struct ixgbevf_ring *rx_ring, u16 cleaned_count)
#endif /* DEV_HPCAP */
 ...
 **/
/*static */void ixgbevf_alloc_rx_buffers_hpcap(struct ixgbevf_ring *rx_ring, u16 cleaned_count) //different from version 2.0.38
{
        union ixgbe_adv_rx_desc *rx_desc;
        unsigned int i = rx_ring->next_to_use;
...
void ixgbevf_alloc_rx_buffers(struct ixgbevf_ring *rx_ring, u16 cleaned_count)
{
        struct ixgbevf_adapter *adapter = rx_ring->adapter;

        if( adapter->work_mode == 2 )
                ixgbevf_alloc_rx_buffers_hpcap(rx_ring, cleaned_count);
        else
                ixgbevf_alloc_rx_buffers_standard(rx_ring, cleaned_count);
}
#endif /* DEV_HPCAP */
 ...
 * q_vector!!!
 **/
int ixgbevf_clean_rxonly(struct napi_struct *napi, int budget)
{
	struct ixgbevf_q_vector *q_vector = container_of(napi, struct ixgbevf_q_vector, napi);
	struct ixgbevf_adapter *adapter = q_vector->adapter;
	struct ixgbevf_ring *rx_ring = q_vector->rx.ring;
	int work_done = 0;

#ifdef IXGBE_DCA
	if (adapter->flags & IXGBE_FLAG_DCA_ENABLED)
		ixgbevf_update_rx_dca(adapter, rx_ring, get_cpu());
#endif
	...
	if(work_done < budget)
	{
		...
		{
			ixgbevf_set_itr(q_vector);//different from version 2.0.38
		}
		...
		if( !rx_ring->wq ) //&& ( adapter->flags2 & IXGBE_FLAG_RX_KERNEL_ENABLE ) )
		{
			if (!test_bit(__IXGBEVF_DOWN, &adapter->state))
			{
				ixgbevf_irq_enable_queues(adapter, ((u64)1 << q_vector->v_idx));
			}
		}
	}
...
static void ixgbevf_napi_enable_all(struct ixgbevf_adapter *adapter)
{
	...
	for (q_idx = 0; q_idx < adapter->num_q_vectors; q_idx++) {
		...
		ixgbevf_qv_init_lock(adapter->q_vector[q_idx]);
			...
			if (adapter->work_mode == 2 )
			{
				adapter->q_vector[q_idx]->napi.poll = &ixgbevf_clean_rxonly;
			}
		#endif /* DEV_HPCAP */
...
int ixgbevf_setup_rx_resources(struct ixgbevf_ring *rx_ring)
{
	...
	#ifdef DEV_HPCAP
		rx_ring->queued = 0;
		size = ALIGN(IXGBEVF_SUBWINDOW_SIZE * MAX_DESCR_SIZE, 4096);
		rx_ring->window_size = size;
                for (i = 0; i <= (rx_ring->count - 1) / IXGBEVF_SUBWINDOW_SIZE; i++)
                {
	...
	alloc_window_failed:
		for (i = 0; i < IXGBEVF_MAX_SUBWINDOWS; i++)
                {
                        if (rx_ring->window[i])
                        {
				   ...
				   const struct pci_device_id *ent)
{
	...
	if (err)
		...
		if( adapter->work_mode == 2 )
			...
			sprintf(netdev->name, "xgb%d", adapter->bd_number);
	#else /* DEV_HPCAP */
