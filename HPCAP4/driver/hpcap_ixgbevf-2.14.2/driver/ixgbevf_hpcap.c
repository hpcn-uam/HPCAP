...
/* Las siguientes dos variables se rellenan en ixgbevf_probe() */
int adapters_found;
struct ixgbevf_adapter * adapters[IXGBEVF_MAX_NIC];
...
//int paqn=0;
u64 hpcap_rx(struct ixgbevf_ring *rx_ring, u64 limit, char *pkt_buf, u64 bufsize, int offs, u64 *fs, u16 caplen
#ifdef REMOVE_DUPS
...
)
{
	...
	unsigned char tmp_h[RAW_HLEN];
		...
		struct ixgbevf_adapter *adapter = rx_ring->adapter; //different from version 2.0.38
	#endif
	...
	prefetchnta(IXGBEVF_RX_DESC(rx_ring, qidx + 0));//different from version 2.0.38
	prefetchnta(IXGBEVF_RX_DESC(rx_ring, qidx + 1));//different from version 2.0.38
	...
	while( cnt < limit )
	{
		...
		{
		#endif
			rx_desc = IXGBEVF_RX_DESC(rx_ring, next_qidx);//different from version 2.0.38
...
ignore:
	...
	#endif
	{
		...
		ixgbevf_release_rx_desc(rx_ring, rx_ring->next_to_use);
...
{
	struct ixgbevf_ring *rx_ring = arg;
...
int hpcap_stop_poll_threads(struct ixgbevf_adapter *adapter)
{
	int i;
...
int hpcap_launch_poll_threads(struct ixgbevf_adapter *adapter)
{
	int i;
...
#endif
int hpcap_buf_init(struct hpcap_buf *bufp, struct ixgbevf_adapter *adapter, int queue,struct cdev *chard, u64 size, int ifnum)
{
	int i;
...
int hpcap_unregister_chardev(struct ixgbevf_adapter *adapter)
{
	int i,major;
	struct hpcap_buf *bufp;
	dev_t dev;
...
int hpcap_register_chardev(struct ixgbevf_adapter *adapter, u64 size, int ifnum)
{
	int i,ret=0,major=0;
	dev_t dev = 0;
